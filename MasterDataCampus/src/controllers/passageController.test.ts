...
describe('passage contoller', function () {
    ...
    it('getPassagesBetween2Buildings: returns json with building1+building2', async function () {
        ...
        sinon.stub(passageServiceInstance, 'getPassagesBetween2Buildings').returns(Result.ok<IPassageDTO[]>([{
             id: 'sad', building1: 'A', building2: 'B', pisobuilding1: 'A1', pisobuilding2: 'B2', positionBuilding1: [], positionBuilding2: [] }]));
        const ctrl = new PassageController(passageServiceInstance as IPassageService);
        await ctrl.getPassagesBetween2Buildings(<Request>req, <Response>res, <NextFunction>next);
        sinon.assert.calledOnce(res.json);
        sinon.assert.calledWith(res.json, sinon.match([{
            id: 'sad',
            building1: 'A',
