...
export default class Maze extends THREE.Group {
        ...
        this.onLoad = function (description) {
            ...
            for (let i = 0; i <= this.size.depth; i++) { // In order to represent the southmost walls, the map depth is one row greater than the actual maze depth
                ...
                for (let j = 0; j <= this.size.width; j++) { // In order to represent the eastmost walls, the map width is one column greater than the actual maze width
                    ...
                    if (this.map[i][j] == 4 || this.map[i][j] == 6) {
                        this.aabb[i][j][0] = new THREE.Box3();
                        for (let k = 0; k < 2; k++) {
                            geometryDoor = door.geometries[k].clone();
                            geometryDoor.applyMatrix4(new THREE.Matrix4().makeTranslation(j - this.halfSize.width + 0.5, 0.25, i - this.halfSize.depth));
                            geometryDoor.computeBoundingBox();
                            geometryDoor.boundingBox.applyMatrix4(new THREE.Matrix4().makeScale(this.scale.x, this.scale.y, this.scale.z));
                            geometriesDoor[k].push(geometryDoor);
                            this.aabb[i][j][0].union(geometryDoor.boundingBox);
                        }
                        this.helper.add(new THREE.Box3Helper(this.aabb[i][j][0], this.helpersColor));
                    }
                    
                    
                    if (this.map[i][j] == 5 ||  this.map[i][j]== 6) {
                        this.aabb[i][j][1] = new THREE.Box3();
                        for (let k = 0; k < 2; k++) {
                            geometryDoor = door.geometries[k].clone();
                            geometryDoor.applyMatrix4(new THREE.Matrix4().makeRotationY(Math.PI / 2.0));
                            geometryDoor.applyMatrix4(new THREE.Matrix4().makeTranslation(j - this.halfSize.width, 0.25, i - this.halfSize.depth + 0.5));
                            geometryDoor.computeBoundingBox();
                            geometryDoor.boundingBox.applyMatrix4(new THREE.Matrix4().makeScale(this.scale.x, this.scale.y, this.scale.z));
                            geometriesDoor[k].push(geometryDoor);
                            this.aabb[i][j][1].union(geometryDoor.boundingBox);
                        }
                        this.helper.add(new THREE.Box3Helper(this.aabb[i][j][1], this.helpersColor));
                    } 

                    if (this.map[i][j] == 7  || this.map[i][j] == 8) {
                        this.aabb[i][j][0] = new THREE.Box3();
                        for (let k = 0; k < 2; k++) {
                            geometryElevator = elevator.geometries[k].clone();
                            geometryElevator.applyMatrix4(new THREE.Matrix4().makeTranslation(j - this.halfSize.width + 0.5, 0.25, i - this.halfSize.depth));
                            geometryElevator.computeBoundingBox();
                            geometryElevator.boundingBox.applyMatrix4(new THREE.Matrix4().makeScale(this.scale.x, this.scale.y, this.scale.z));
                            geometriesElevator[k].push(geometryElevator);
                            this.aabb[i][j][0].union(geometryElevator.boundingBox);
                        }
                        this.helper.add(new THREE.Box3Helper(this.aabb[i][j][0], this.helpersColor));
                    }
                }
            ...
            let mergedGeometryDoor, meshDoor;
            for (let i = 0; i < 2; i++) {
                mergedGeometryDoor = BufferGeometryUtils.mergeGeometries(geometriesDoor[i], false);
                meshDoor = new THREE.Mesh(mergedGeometryDoor, door.materials[i]);
                meshDoor.castShadow = true;
                meshDoor.receiveShadow = true;
                this.add(meshDoor);
            }
            let mergedGeometryElevator, meshElevator;
            for (let i = 0; i < 2; i++) {
                mergedGeometryElevator = BufferGeometryUtils.mergeGeometries(geometriesElevator[i], false);
                meshElevator = new THREE.Mesh(mergedGeometryElevator, elevator.materials[i]);
                meshElevator.castShadow = true;
                meshElevator.receiveShadow = true;
                this.add(meshElevator);
            }
    ...
    doorcornerCollision(indices, offsets, orientation, position, delta, radius, name) {
        const row = indices[0] + offsets[0];
        const column = indices[1] + offsets[1];
        if (this.map[row][column] == 4 - orientation || this.map[row][column] == 5) {
            const x = position.x - (this.cellToCartesian([row, column]).x + delta.x * this.scale.x);
            const z = position.z - (this.cellToCartesian([row, column]).z + delta.z * this.scale.z);
            if (x * x + z * z < radius * radius) {
                console.log("Collision with " + name + ".");
    ...
    collision(method, position, halfSize, direction) {
        ...
        if (method != "obb-aabb") {
            if (
                ...
                this.doorcornerCollision(indices, [1, 0], 1, position, { x: -0.475, z: -0.5 }, halfSize, "southwest corner (NS-oriented wall)") || // Collision with southwest corner (NS-oriented wall)
                this.doorcornerCollision(indices, [1, 1], 0, position, { x: -0.5, z: -0.525 }, halfSize, "southeast corner (WE-oriented wall)") || // Collision with southeast corner (WE-oriented wall)
                this.doorcornerCollision(indices, [1, 1], 1, position, { x: -0.525, z: -0.5 }, halfSize, "southeast corner (NS-oriented wall)") || // Collision with southeast corner (NS-oriented wall)
                this.doorcornerCollision(indices, [0, 1], 0, position, { x: -0.5, z: -0.475 }, halfSize, "northeast corner (WE-oriented wall)") || // Collision with northeast corner (WE-oriented wall)
                //this.doorCollision(indices, [0, 0], 0, position, { x: 0.0, z: -0.475 }, halfSize, "north door") ||
                this.doorCollision(indices, [0, 0], 1, position, { x: -0.475, z: 0.0 }, halfSize, "west door") ||
                //this.doorCollision(indices, [1, 0], 0, position, { x: 0.0, z: -0.525 }, halfSize, "south door") || 
                ...
                indices[0] > 0 && (
                    ...
                    this.doorcornerCollision(indices, [-1, 1], 1, position, { x: -0.525, z: 0.5 }, halfSize, "northeast corner (NS-oriented wall)") || // Collision with northeast corner (NS-oriented wall)
                    this.doorcornerCollision(indices, [-1, 0], 1, position, { x: -0.475, z: 0.5 }, halfSize, "northwest corner (NS-oriented wall)") // Collision with northwest corner (NS-oriented wall)
                ) ||
                indices[1] > 0 && (
                    ...
                    this.doorcornerCollision(indices, [0, -1], 0, position, { x: 0.5, z: -0.475 }, halfSize, "northwest corner (WE-oriented wall)") || // Collision with northwest corner (WE-oriented wall)
                    this.doorcornerCollision(indices, [1, -1], 0, position, { x: 0.5, z: -0.525 }, halfSize, "southwest corner (WE-oriented wall)") // Collision with southwest corner (WE-oriented wall)
                )
            ) {
                return true;
            }
            ...
            if (
                ...
                this.doorAndCornerCollision(indices, [0, 0], 0, obb, "north wall") || // Collision with north wall
                this.doorAndCornerCollision(indices, [0, 0], 1, obb, "west wall") || // Collision with west wall
                this.doorAndCornerCollision(indices, [1, 0], 0, obb, "south wall") || // Collision with south wall
                this.doorAndCornerCollision(indices, [0, 1], 1, obb, "east wall") || // Collision with east wall
                ...
                this.doorAndCornerCollision(indices, [1, 0], 1, obb, "southwest corner (NS-oriented wall)") || // Collision with southwest corner (NS-oriented wall)
                this.doorAndCornerCollision(indices, [1, 1], 0, obb, "southeast corner (WE-oriented wall)") || // Collision with southeast corner (WE-oriented wall)
                this.doorAndCornerCollision(indices, [1, 1], 1, obb, "southeast corner (NS-oriented wall)") || // Collision with southeast corner (NS-oriented wall)
                this.doorAndCornerCollision(indices, [0, 1], 0, obb, "northeast corner (WE-oriented wall)") || // Collision with northeast corner (WE-oriented wall)
                ...
                indices[0] > 0 && (
                    ...
                    this.doorAndCornerCollision(indices, [-1, 1], 1, obb, "northeast corner (NS-oriented wall)") || // Collision with northeast corner (NS-oriented wall)
                    this.doorAndCornerCollision(indices, [-1, 0], 1, obb, "northwest corner (NS-oriented wall)") // Collision with northwest corner (NS-oriented wall)
                ) ||
                indices[1] > 0 && (
                    ...
                    this.doorAndCornerCollision(indices, [0, -1], 0, obb, "northwest corner (WE-oriented wall)") || // Collision with northwest corner (WE-oriented wall)
                    this.doorAndCornerCollision(indices, [1, -1], 0, obb, "southwest corner (WE-oriented wall)") // Collision with southwest corner (WE-oriented wall)
             )
            ) {
                return true;
            }
