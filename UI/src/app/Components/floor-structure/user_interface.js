...
export default class UserInterface extends GUI {
    constructor(thumbRaiser) {
        ...
        const buildingsFolder = this.addFolder("Buildings");
        buildingsFolder.domElement.style.fontSize = fontSize;
        const buildingParameters = { selectedBuilding: "Floor A1" };
        const buildingOptions = ["Floor A1", "Floor A2", "Floor B1"];
        buildingsFolder.add(buildingParameters, "selectedBuilding").options(buildingOptions).onChange(selectedBuilding => {
            console.log("Selected Building:", selectedBuilding);
            const newUrl = getBuildingUrl(selectedBuilding);
            console.log("vai entrar no thumbRaiser")
            thumbRaiser.reloadScene(newUrl);
        });
        
        buildingsFolder.close();


        function getBuildingUrl(selectedBuilding) {

            const buildingUrls = {
                "Floor A1": "./assets/mazes/Loquitas_10x10_displacement.json",
                "Floor A2": "./assets/mazes/Loquitas_20x20.json",
                "Floor B1": "./assets/mazes/Loquitas_20x20_displacement.json",
            };
            return buildingUrls[selectedBuilding] || "";
        }
